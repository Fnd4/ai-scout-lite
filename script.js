/* ─────────────────────────────────────────
   ...
   Demo mode: fake data, no API calls yet.
   Replace analyzeFake() with analyzeReal()
   once Netlify function is ready.
───────────────────────────────────────── */

// ── DEMO DATA POOL ──────────────────────
// Multiple fake results so repeated clicks
// don't look completely broken.
const DEMO_RESULTS = [
  {
    scoutScore: 74,
  ...
  },
  {
    ...
    weaknesses: "Calorie estimation from photos has a well-documented ~30% error rate. The App Store already has Lose It, Calorie Mama, and Google Lens doing this for free. No sticky mechanic — why would a user return? Regulatory exposure if health claims are made.",
  ...
  {
    scoutScore: 61,
    summary: "A Slack bot that monitors team messages and surfaces a weekly 'communication health' report — identifies who's being excluded from key conversations, flags passive-aggressive tone patterns, and suggests who should be looped in on which threads.",
    whoItHelps: "Engineering managers and team leads at companies above 20 people. HR teams dealing with distributed team dynamics. Could also appeal to founders who want visibility without reading every Slack channel.",
    weaknesses: "Significant privacy concerns — employees will resist being sentiment-analyzed. Requires deep Slack permissions that security teams routinely block. Risk of false positives creating conflict rather than resolving it. Needs careful legal review before enterprise sales.",
    scores: { usefulness: 18, originality: 19, technical: 16, beginner: 7 },
    verdict: "WATCH",
    nextStep: "Reframe the product around opt-in team retrospectives rather than passive surveillance. Give individuals visibility into their own communication patterns first. Trust is the moat here — earn it before selling to management."
  }
];
...
analyzeBtn.addEventListener('click', async () => {
  ...
  // Simulate async analysis delay (replace with real fetch later)
...
// ── DEMO DATA GETTER ─────────────────────
// Cycles through pool so each click looks different
function getDemo() {
...
/* ─────────────────────────────────────────
   FUTURE: Replace getDemo() with this
   once netlify/functions/analyze.js exists:
